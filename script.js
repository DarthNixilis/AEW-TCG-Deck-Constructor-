document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE MANAGEMENT ---
    let cardDatabase = [];
    let startingDeck = []; // Now stores only card IDs
    let purchaseDeck = []; // Now stores only card IDs
    let selectedWrestler = null;
    let selectedManager = null;
    let activeFilters = [{}, {}, {}];
    let currentViewMode = 'list';
    let lastFocusedElement; // For modal accessibility
    ...
    async function loadCardDatabase() {
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) throw new Error('Card database file not found (404). Please ensure cardDatabase.json exists in the correct location.');
            ...
            searchResults.innerHTML = `<p style="color: red;"><strong>Error:</strong> ${error.message}<br>Please check the developer console (F12) for more details.</p>`;
        }
    }
    ...
    function addDeckSearchFunctionality() {
        ...
        purchaseDeckSearch.className = 'deck
